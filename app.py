...
if page == "航线规划":
    ...
    with col1:
        ...
        if st.button("设置A点", key="set_a_btn"):
    ...
    with col2:
        ...
        if st.button("设置B点", key="set_b_btn"):
            if coord_system == "WGS-84":
            ...
            st.success("B点设置成功！")
    # 飞行高度设置（加 key）
    height = st.slider("设定飞行高度(m)", min_value=10, max_value=200, value=50, key="height_slider")
...
if page == "飞行监控":
    st.title("✈️ 飞行监控（心跳包）")
    # 模拟心跳包上传（加 key）
    if st.button("上传测试心跳包", key="upload_heartbeat_btn"):
        test_data = {
            "lat": 32.233, 
            "lng": 118.749, 
            "height": 50, 
    ...
    # 显示心跳包数据（加 key）
    st.subheader("心跳包历史")
    st.dataframe(st.session_state.drone_data["heartbeat"], key="heartbeat_df")
